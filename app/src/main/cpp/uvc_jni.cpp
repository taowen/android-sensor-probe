...
extern "C" JNIEXPORT jlong JNICALL Java_io_github_sensorprobe_UvcNative_start(JNIEnv*, jobject, jint fd) {
    auto* u=new Uvc();
    ...
    __android_log_print(ANDROID_LOG_INFO,"SensorProbeUVC","format=%u frame=%u interval=%u maxFrame=%u payload=%u alt=%d",
        probe[2],probe[3],le32(probe+4),le32(probe+18),le32(probe+22),u->altSetting);
...
extern "C" JNIEXPORT jbyteArray JNICALL Java_io_github_sensorprobe_UvcNative_readFrame(JNIEnv* env,jobject,jlong p) {
...
extern "C" JNIEXPORT void JNICALL Java_io_github_sensorprobe_UvcNative_stop(JNIEnv*,jobject,jlong p) {
